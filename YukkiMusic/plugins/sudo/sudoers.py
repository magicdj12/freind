...
async def useradd(client, message: Message, _):
    if MONGO_DB_URI is None:
        return await message.reply_text(
            "**به دلیل مشکلات حریم خصوصی ربات، شما نمی‌توانید کاربران سادو را هنگام استفاده از پایگاه داده یاکی مدیریت کنید.**"
        )
...
async def userdel(client, message: Message, _):
    if MONGO_DB_URI is None:
        return await message.reply_text(
            "**لطفاً MONGO_DB_URI خود را در متغیرهای خود پر کنید تا از این ویژگی استفاده کنید.**"
        )
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.reply_text(_["general_1"])
        ...
        if removed:
            ...
            return
        await message.reply_text(f"یک اتفاقی رخ داده است")
        return
