    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            if not config.SPOTIFY_CLIENT_ID and not config.SPOTIFY_CLIENT_SECRET:
                return await mystic.edit_text(
                    "این ربات نمی‌تواند ترک‌ها و لیست‌های پخش اسپاتیفای را پخش کند، لطفاً با صاحب من تماس بگیرید و از او بخواهید تا پلیر اسپاتیفای را اضافه کند."
                )
            if "track" in url:
                try:
            ...
            if not await Telegram.is_streamable_url(url):
                return await mystic.edit_tex(
                    "اوه! فکر نمی‌کنم که این یک URL قابل پخش باشد."
                )
...
async def play_music(client, CallbackQuery, _):
    ...
    ffplay = True if fplay == "ا" else None
    try:
        await stream(
            _,
            mystic,
...
@app.on_callback_query(filters.regex("PyKalierPlaylists") & ~BANNED_USERS)
@languageCB
