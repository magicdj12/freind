    ...
    try:
        try:
            ...
            return await CallbackQuery.answer(
                f"» من اجازه ندارم که کاربران را از طریق لینک دعوت کنم. {CallbackQuery.message.chat.title}.",
                show_alert=True,
            )
        if get.status == ChatMemberStatus.BANNED:
            return await CallbackQuery.answer(
                text=f"»دستیار صوتی منع شده است {CallbackQuery.message.chat.title}",
                show_alert=True,
            )
            ...
            try:
                ...
                try:
                    ...
                    return await CallbackQuery.message.reply_text(
                        f"دعوت کردن دستیار صوتی به شکست مواجه شد {CallbackQuery.message.chat.title}\nدلیل :{e}"
                    )
            except Exception as ex:
...
async def play_playlist_command(client, message, _):
    msg = await message.reply_text("لطفا چند لحظه صبر کنید....")
    try:
        try:
            ...
            return await msg.edit_text(
                f"» من اجازه ندارم که کاربران را از طریق لینک دعوت کنم. {userbot.mention} دستایر صوتی به {message.chat.title}."
            )
        if get.status == ChatMemberStatus.BANNED:
            return await msg.edit_text(
                text=f"» {userbot.mention} دستیار صوتی منع شده است {message.chat.title}\n\n𖢵 ایدی : `{userbot.id}`\n𖢵 نام : {userbot.mention}\n𖢵 نام کاربری : @{userbot.username}\n\nلطفا دستیار صوتی را آزاد کنید و دوباره پخش کنید...",
            )
    except UserNotParticipant:
            ...
            try:
                ...
                try:
                    ...
                    return await msg.edit(
                        f"دعوت کردن به شکست مواجه شدتت {userbot.mention} دستایر صوتی به {message.chat.title}.\n\n**دلیل :** `{ex}`"
                    )
            except Exception as ex:
                if "channels.JoinChannel" in str(ex) or "Username not found" in str(ex):
                    return await msg.edit_text(
                    ...
                    return await msg.edit_text(
                        f"دعوت کردن به شکست مواجه شد {userbot.mention} دستایر صوتی به {message.chat.title}.\n\n**دلیل :** `{ex}`"
                    )
        if invitelink.startswith("https://t.me/+"):
            invitelink = invitelink.replace("https://t.me/+", "https://t.me/joinchat/")
        anon = await msg.edit_text(
            f"لطفا صبر کنید...\n\nدعوت  {userbot.mention} ᴛᴏ {message.chat.title}."
        )
        try:
            await userbot.join_chat(invitelink)
            await asyncio.sleep(2)
            await msg.edit_text(
                f"{userbot.mention} موفقانه جوین شد,\n\nدرحال شروع پخش ..."
            )
        except UserAlreadyParticipant:
            pass
        except InviteRequestSent:
...
@app.on_message(
    filters.command(["playgplaylist", "vplaygplaylist"],prefixes=['','/']) & ~BANNED_USERS & filters.group
)
@languageCB
async def play_playlist_command(client, message, _):
    mode = message.command[0][0]
...
@app.on_message(filters.command(["addplaylist"],prefixes=['','/']) & ~BANNED_USERS)
@language
async def add_playlist(client, message: Message, _):
    if len(message.command) < 2:
        return await message.reply_text(
            "لطفاً بعد از دستور، نام آهنگ یا لینک آهنگ یا لینک پلی‌لیست یوتیوب را به من بدهید.مثال‌ها:▷ /addplaylist Ram siya ram (یک نام آهنگ خاص قرار دهید)▷ /addplaylist [لینک پلی‌لیست یوتیوب] (برای اضافه کردن تمام آهنگ‌های یک پلی‌لیست یوتیوب به پلی‌لیست ربات)")

    query = message.command[1]

    # Check if the provided input is a YouTube playlist link
    if "youtube.com/playlist" in query:
        adding = await message.reply_text("** درحال اضافه کردن به لیست پخش..**")
        try:
        ...
        if not video_urls:
            return await message.reply_text(
                "**در لیست پخش چیزی پیدا نشد.\n\n**لطفا لیست های پخش دیگر را ببینید**"
            )
        ...
        for video_url in video_urls:
            ...
            try:
                ...
                return await message.reply_text(f"خطا در گرفتن مشخصات ویدیو: {e}")
        ...
        keyboardes = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text="๏ میخوهید موزیک را حذف کنید ? ๏",
                        url=f"https://t.me/{app.username}?start=delplaylists",
                    ),
                ]
            ]
