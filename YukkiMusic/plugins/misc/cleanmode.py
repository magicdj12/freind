...
async def clean_mode(client, update, users, chats):
    ...
    put = {
        ...
        "بعد مدتی": time_now + timedelta(minutes=AUTO_DELETE),
    }
...
__MODULE__ = "ارسال همگانی"
__HELP__ = """
<b>/broadcast [پیام یا پاسخ به یک پیام]</b> » پیام را به چت‌های فعال ربات ارسال کنید.
<u>حالت‌های پخش:</u>

<b><code>-pin</code></b> » پیام‌های پخش شده شما را در چت‌های فعال پین می‌کند.

<b><code>-pinloud</code></b> » پیام پخش شده شما را در چت‌های فعال پین کرده و به اعضا اعلان ارسال می‌کند.

<b><code>-user</code></b> » پیام را به کاربران ارسال می‌کند که ربات شما را استارت کرده‌اند.

<b><code>-assistant</code></b> » پیام شما را از حساب دستیار ربات ارسال می‌کند.

<b><code>-nobot</code></b> » ربات را مجبور می‌کند تا پیام را پخش نکند.

> <b>مثال:</b> <code>/broadcast -user -assistant -pin تست پخش</code>
